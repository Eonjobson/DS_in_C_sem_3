...
**/
Node* insert(Node* head,Node* prev,int val,int index){
	if(head==NULL){
		new_node = create(val,NULL);
		size++;
		return new_node;
	}
	if(index==0){
		 new_node = create(val,prev);
		 new_node->next = head;
		 head->prev = new_node;
		 size++;
		 return new_node;
	}
	head->next = insert(head->next,head,val,index-1);
	return head;
}
    ...
    while(1)
    {
        ...
        switch(choice)
        {
            case 1: 
		        ...
		        int ni;
		        scanf("%d",&ni);
		        head= insert(head,head,ni,0);
            		break;
            case 2: 
            		printf("\nEnter value\t");
		        int ne;
		        scanf("%d",&ne);
		        head= insert(head,head,ne,size);
            		break;
            case 3: 
            		printf("\nEnter value\t");
		        int n,idx;
